...
function state:init()
  ...
  self.items = {
    ...
    {"Restart song", function()
      gamestate.switch(game, self.previous.filename, self.previous.song, self.previous.audioData)
    end},
    {"Play another song", function()
      songselect:run(function(filename, song, data)
        gamestate.switch(game, filename, song, data)
      end)
    end},
    {"Back to menu", function() gamestate.switch(menu) end},
...
function state:mousepressed(x, y, button)
  ...
  if button == 1 then
    ...
    if math.abs(i) > 300 * self.visibility and math.abs(j) > 200 * self.visibility then
      self.closing = true
